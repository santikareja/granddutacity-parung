...
export async function up(args: MigrateUpArgs): Promise<void> {
  ...
  const db = (args as MigrateUpArgs & { db: { execute: (query: unknown) => Promise<unknown> } }).db
  // 1. Ensure tags exist
  const tagsToEnsure = [
    { name: 'Agen', slug: 'agen' },
  ...
  let mediaId: number | undefined = mediaRes.docs[0]?.id as (number | undefined)
  if (!mediaId) {
    const fallbackMedia = await payload.find({ collection: 'media', limit: 1, req })
    mediaId = fallbackMedia.docs[0]?.id as (number | undefined)
  }

  if (!mediaId) {
    payload.logger.warn('No media found. Inserting minimal media record through SQL for required relation.')
    ...
    await db.execute(sql`
      ...
      VALUES (
        ...
        'https://res.cloudinary.com/dzhvfbuks/image/upload/v1776585039/10_Ciri_Agen_Properti_Terbaik_qnwhni.webp',
        '10-ciri-agen-properti-terbaik.webp',
        'image/webp',
        now(),
        now()
      )
    ...
    const mediaAfterInsert = await payload.find({
      collection: 'media',
      where: { filename: { equals: '10-ciri-agen-properti-terbaik.webp' } },
      limit: 1,
      req,
    })

    mediaId = mediaAfterInsert.docs[0]?.id as (number | undefined)
  }

  if (!mediaId) {
    throw new Error('Cannot resolve featured image for article migration.')
  }
